...
/// modifications.
public struct OrganizationEvent: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  ...
  public init(from decoder: Decoder) throws {
    ...
    if let value = try container.decodeIfPresent(
      OrganizationEvent.EventCategory.self, forKey: .category)
    {
      self.category = value
    }
    if let value = try container.decodeIfPresent(
      OrganizationEvent.DetailedCategory.self, forKey: .detailedCategory)
    {
      self.detailedCategory = value
    }
    if let value = try container.decodeIfPresent(OrganizationEvent.State.self, forKey: .state) {
      self.state = value
    }
    if let value = try container.decodeIfPresent(
      OrganizationEvent.DetailedState.self, forKey: .detailedState)
    {
